...
//    calcDimentionFeature(); // 次元特徴
    ...
    calcChangeOfCurvature(); // Change of Curvature
    calcPlanarity(); // Planarity
    calcAplanarity(); // Aplanarity
    calcLinearity(); // Linearity
}
...
void PCA::drawEigenvectors(const ofVec3f& targetPos) {
    ...
    ofSetColor(255, 0, 0);
    ofDrawLine(targetPos, targetPos+eigenVectors[0]*50);
    
    ofSetColor(0, 255, 0);
    ofDrawLine(targetPos, targetPos+eigenVectors[1]*50);
    
    ofSetColor(0, 0, 255);
    ofDrawLine(targetPos, targetPos+eigenVectors[2]*50);
}

void PCA::normalizeEigenVector() {
...
// Change of Curvature
void PCA::calcChangeOfCurvature() {
    changeOfCurvature = eigenValues[2] / (eigenValues[0] + eigenValues[1] + eigenValues[2]);
}

// Planarity
void PCA::calcPlanarity() {
    planarity = (eigenValues[1] - eigenValues[2]) / eigenValues[0];
}

// Aplanarity
void PCA::calcAplanarity() {
    aplanarity = 1.0 - (eigenValues[1] - eigenValues[2]) /eigenValues[0];
}

// Linearity
void PCA::calcLinearity() {
    // (1 - 2) / 1
...
float PCA::getChangeOfCurvature() const{
    return changeOfCurvature;
}

float PCA::getPlanarity() const{
    return planarity;
}

float PCA::getAplanarity() const{
    return aplanarity;
}

float PCA::getLinearity() const{
    return linearity;
}
