...
void setViridis(ofFloatColor* viridis);
...
void ofApp::setup(){
    ...
    // PLYファイルの読み込み
    //vboMesh.load("ply/bunny.ply");
    //vboMesh.load("ply/heritage.ply");
    //vboMesh.load("ply/ginkakuji.ply");
    //vboMesh.load("ply/ginkakuji10000.ply");
    //vboMesh.load("ply/ginkakuji30000.ply");
    //vboMesh.load("ply/yatai.ply");
    vboMesh.load("ply/dragon100000.ply");
...
//    int size = 200;
//    int diff = 3;
//    for (int diff_z = 0; diff_z < size; diff_z+=diff ) {
//        for (int diff_y = 0; diff_y < size; diff_y+=diff ) {
//            for (int diff_x = 0; diff_x < size; diff_x+=diff ) {
//                if ( diff_x == 0 || diff_y == 0 || diff_z == 0 ) {
//                    float x = (float)diff_x - (float)size*0.5;
//                    float y = (float)diff_y - (float)size*0.5;
//                    float z = (float)diff_z - (float)size*0.5;
...
//    vboMesh.addVertices( vecs );
    ...
    setViridis(viridis);
//    for (int i = 0; i < 256; i++) {
//        std::cout << i << " : " << viridis[i] << std::endl;
//    }
    rainbowImage.load("images/Viridis.png");
    
    // 特徴量の値を出力するファイル
    oldFeatureValue.open("oldFeatureValues.txt", ofFile::WriteOnly);
    newFeatureValue.open("newFeatureValues.txt", ofFile::WriteOnly);
}
...
void ofApp::setGUI() {
    ...
    gui.add(isDrawAxis.setup("Draw Axis", false));
    gui.add(isDrawFrame.setup("Draw Frame", false));
    ...
    gui5.add(countSphereRadius.setup("Sphere Radius", diagonalLineLength*0.03, 0, diagonalLineLength*0.1));
    ...
    gui5.add(VisualizeCurvature.setup("Visualize Curvature"));
    gui5.add(VisualizePlanarity.setup("Visualize Planarity"));
    gui5.add(VisualizeAplanarity.setup("Visualize Aplanarity"));
    gui5.add(VisualizeLinearity.setup("Visualize Linearity"));
    ...
    if (isTargetRangePCAMode) {
        ...
        if (PrincipalComponentAnalysis) {
            if (isSelectRadiusRange_5 && !isAfterPCA_5) {
                ...
                changeOfCurvature = pca.getChangeOfCurvature();
                planarity = pca.getPlanarity();
                aplanarity = pca.getAplanarity();
                linearity = pca.getLinearity();
        ...
        if (VisualizeCurvature) {
            isVisualizeChangeOfCurvature = true;
            isVisualizePlanarity = false;
            isVisualizeAplanarity = false;
            isVisualizeLinearity = false;
            changeFeatureValueDistribution(vboMesh);
        }
        
        if (VisualizePlanarity) {
            isVisualizeChangeOfCurvature = false;
            isVisualizePlanarity = true;
            isVisualizeAplanarity = false;
            isVisualizeLinearity = false;
            changeFeatureValueDistribution(vboMesh);
        }
        
        if (VisualizeAplanarity) {
            isVisualizeChangeOfCurvature = false;
            isVisualizePlanarity = false;
            isVisualizeAplanarity = true;
            isVisualizeLinearity = false;
            changeFeatureValueDistribution(vboMesh);
        }
        
        if (VisualizeLinearity) {
            isVisualizeChangeOfCurvature = false;
            isVisualizePlanarity = false;
            isVisualizeAplanarity = false;
            isVisualizeLinearity = true;
            changeFeatureValueDistribution(vboMesh);
        }
...
//    ofSetColor(255);
    ...
    if (isTargetRangePCAMode && !isInterPointDistMode && !isRemovePointsMode && !isCreateCrossSectionMode && !isInterpolatePointsMode ) {
        ...
        // 可視化後
        if (isFirstFeatureValue) {
            // 注目点の特徴量の表示
            ofSetColor(255, 0, 0);
            if ( isVisualizeChangeOfCurvature ) {
                verdana30.drawString("Change of Curvature : "+ofToString(changeOfCurvatures[nearestIndex]), mousePos.x+30, mousePos.y+100);
            } else if ( isVisualizePlanarity ) {
                verdana30.drawString("Planarity : "+ofToString(planarities[nearestIndex]), mousePos.x+30, mousePos.y+100);
            } else if ( isVisualizeAplanarity ) {
                verdana30.drawString("Aplanarity : "+ofToString(aplanarities[nearestIndex]), mousePos.x+30, mousePos.y+100);
                
            } else if ( isVisualizeLinearity ) {
                if (isFirstFeatureValue && !isSecondFeatureValue) {
                    verdana30.drawString("Linearity : "+ofToString(linearities[nearestIndex]), mousePos.x+30, mousePos.y+100);
                }
                
                if (isSecondFeatureValue) {
                    verdana30.drawString("Linearity(Old) : "+ofToString(linearities[nearestIndex]), mousePos.x+30, mousePos.y+100);
                    verdana30.drawString("Linearity(New) : "+ofToString(newLinearities[nearestIndex]), mousePos.x+30, mousePos.y+140);
                }
            }
            ...
            // レインボー画像表示
            ofSetColor(255);
            rainbowImage.draw(ofGetWidth()*0.5-rainbowImage.getWidth()*0.5, ofGetHeight()-150, rainbowImage.getWidth(), rainbowImage.getHeight()*0.8);
        }
    ...
    verdana40.drawString(ofToString(vboMesh.getNumVertices())+" Points", ofGetWidth()*0.44, ofGetHeight()-50);
...
//        }
    ...
    if (key == 'p') {
        if (isSelectRadiusRange_5 && !isAfterPCA_5) {
            ...
            planarity = pca.getPlanarity();
            aplanarity = pca.getAplanarity();
            linearity = pca.getLinearity(); // Linearityを取得
    ...
    if (key == 'v') {
        ...
        // 特徴量の分布を可視化
        if (!isFirstFeatureValue) {
            visualizeFeatureValueDistribution(vboMesh);
            isFirstFeatureValue = true;
        }
     
        // 2回目の'v'キー押下
        // 1度計算した特徴量を再定義
        else if (isFirstFeatureValue) {
            redefineFeatureValues(vboMesh);
            isSecondFeatureValue = true;
        }
    }
...
void ofApp::searchPointsInSphere(const ofVec3f& queryPointPos, int mode) {
    ...
    if (mode == 4) {
        ...
        pointsInSphere.clear();
        pointsInSphere.reserve(k);
        indexesInSphere.clear();
        indexesInSphere.reserve(k);
        
        for (int i = 1; i < k-1; i++) {
            ofVec3f tmp;
            tmp.x = points[indexes[i]][0]; // [0]:x
            tmp.y = points[indexes[i]][1]; // [0]:y
            tmp.z = points[indexes[i]][2]; // [0]:z
            pointsInSphere.push_back(tmp);
            
            // indexも保存
            indexesInSphere.push_back(indexes[i]);
        }
    }
}
...
// 特徴量の分布を可視化する関数
void ofApp::visualizeFeatureValueDistribution(ofVboMesh& PointCloud) {
    // 点群の色情報をリセット
    PointCloud.getColors().resize( PointCloud.getNumVertices() );
//    changeOfCurvatures.clear(); mappedChangeOfCurvatures.clear();
//    planarities.clear();        mappedPlanarities.clear();
//    aplanarities.clear();       mappedAplanarities.clear();
    linearities.clear();        mappedLinearities.clear();
//    changeOfCurvatures.reserve(PointCloud.getNumVertices());
//    planarities.reserve(PointCloud.getNumVertices());
//    aplanarities.reserve(PointCloud.getNumVertices());
    linearities.reserve(PointCloud.getNumVertices());
//    mappedChangeOfCurvatures.reserve(PointCloud.getNumVertices());
//    mappedPlanarities.reserve(PointCloud.getNumVertices());
//    mappedAplanarities.reserve(PointCloud.getNumVertices());
    mappedLinearities.reserve(PointCloud.getNumVertices());
    
    ////////// 全点群について，特徴量の計算を行う //////////
    // 各点について，計数球内の点に対してPCAをして，特徴量を計算
    for (int i = 0 ; i < PointCloud.getNumVertices(); i++) {
        // 計数球の半径を決める必要
        
        // i番目の点について，計数球内の点群を探索・取得
        searchPointsInSphere(PointCloud.getVertex(i), 5);
        ofVboMesh targetPointCloud;
        for (int j = 0; j < pointsInSphere.size(); j++) {
            targetPointCloud.addVertex( pointsInSphere[j] );
        }
        ...
        // 各特徴量の取得
        changeOfCurvatures.push_back( pca.getChangeOfCurvature() );
//        planarities.push_back( pca.getPlanarity() );
//        aplanarities.push_back( pca.getAplanarity() );
//        linearities.push_back( pca.getLinearity() );
//
//        if ( std::isnan(linearities[i]) ) {
//            linearities[i] = 0.0;
//            cout << i << endl;
//        }
    }
...
//    ////////// 各特徴量の最大・最小を取得////////
//    // 最大値取得
    float maxCurvature = *max_element(changeOfCurvatures.begin(), changeOfCurvatures.end());
//    float maxPlanality = *max_element(planarities.begin(), planarities.end());
//    float maxAplanarity = *max_element(aplanarities.begin(), aplanarities.end());
//    float maxLinearity = *max_element(linearities.begin(), linearities.end());
//    // 最小値取得
//    float minCurvature = *min_element(changeOfCurvatures.begin(), changeOfCurvatures.end());
//    float minPlanality = *min_element(planarities.begin(), planarities.end());
//    float minAplanarity = *min_element(aplanarities.begin(), aplanarities.end());
//    float minLinearity = *min_element(linearities.begin(), linearities.end());

    ////////// 可視化（色付け） //////////
    for (int i = 0 ; i < PointCloud.getNumVertices(); i++) {
        // 各特徴量（0.0〜1.0）を 0.0〜255.0 に正規化
//        float mappedC = ofMap(changeOfCurvatures[i], minCurvature, maxCurvature, 0.0, 255.0);
//        float mappedP = ofMap(planarities[i], minPlanality, maxPlanality, 0.0, 255.0);
//        float mappedA = ofMap(aplanarities[i], minAplanarity, maxAplanarity, 0.0, 255.0);
//        float mappedL = ofMap(linearities[i], minLinearity, maxLinearity, 0.0, 255.0);
//        float mappedC = ofMap(changeOfCurvatures[i], 0.0, 1.0, 0.0, 255.0);
//        float mappedP = ofMap(planarities[i], 0.0, 1.0, 0.0, 255.0);
//        float mappedA = ofMap(aplanarities[i], 0.0, 1.0, 0.0, 255.0);
        float tmp = changeOfCurvatures[i] / maxCurvature;
        //float tmp = linearities[i] / maxLinearity;
        float mappedF = ofMap(tmp, 0.0, 1.0, 0.0, 255.0);
        //col = colorMap.use( (int)mappedChangeOfCurvatures[i] );
        //col = colorMap.use( (int)mappedPlanarities[i] );
        //col = colorMap.use( (int)mappedAplanarities[i] );
        //PointCloud.setColor(i, colorMap.use( (int)mappedLinearities[i] ));
        //PointCloud.setColor(i, colorMap.use( (int)mappedL ));
        PointCloud.setColor( i, viridis[(int)mappedF] );
        
//        // 抽出範囲テスト
//        if ( 0.62 <= linearities[i] && linearities[i] < 0.95 ) {
//            PointCloud.setColor(i, ofFloatColor(1.0, 0.0, 0.0));
//            //PointCloud.setColor(i, ofFloatColor(0.0, 0.0, 0.0, 1.0));
//        } else if ( 0.45 < linearities[i] && linearities[i] < 0.55) {
//            PointCloud.setColor(i, ofFloatColor(1.0, 1.0, 0.0, 1.0));
//            //PointCloud.setColor(i, ofFloatColor(0.0, 0.0, 0.0, 1.0));
//        } else {
//            PointCloud.setColor(i, ofFloatColor(0.0, 0.0, 1.0));
//            //PointCloud.setColor(i, ofFloatColor(0.0, 0.0, 0.0, 1.0));
...
//
        ...
        mappedChangeOfCurvatures.push_back(mappedF);
//        mappedPlanarities.push_back(mappedF);
//        mappedAplanarities.push_back(mappedF);
        //mappedLinearities.push_back(mappedF);
        //normalizedLinearities.push_back(tmp);
    }
    
    //cout << mappedLinearities.size() << endl;
}

void ofApp::redefineFeatureValues(ofVboMesh& PointCloud) {
    // 点群の色情報をリセット
    PointCloud.getColors().resize( PointCloud.getNumVertices() );
    newLinearities.reserve(PointCloud.getNumVertices());
    ...
    for (int i = 0; i < PointCloud.getNumVertices(); i++) {
        ...
        // i番目の点の計数球内の点群に対しての処理
        float tmpSum = 0.0;
        float tmpSum_dF = 0.0;
        for (int j = 0; j < pointsInSphere.size(); j++) {
            float D = PointCloud.getVertex(i).distance(pointsInSphere[j]);
            float F_old = linearities[ indexesInSphere[j] ];
            
            // 特徴量の差を計算
            float dF = linearities[i] - linearities[ indexesInSphere[j] ];
...
//            }
            ...
            tmpSum    += F_old*F_old * (countSphereRadius/D)*(countSphereRadius/D);
            tmpSum_dF += dF; // 特徴量の差の総和
        }
    
        // 特徴量を再定義
        float self = countSphereRadius*linearities[i]; // 自分のFに重み
        float F_new = (tmpSum+self) / (float)pointsInSphere.size(); // 平均
        //float F_new = tmpSum; // 平均しない
        newLinearities.push_back(F_new);
    }
    
    ////////// 再定義した特徴量の最大・最小を取得 ////////
    float maxNewLinearity = *max_element(newLinearities.begin(), newLinearities.end());
    float minNewLinearity = *min_element(newLinearities.begin(), newLinearities.end());
    
    ////////// 可視化（色付け） //////////
    mappedNewLinearities.clear();
    mappedNewLinearities.reserve(PointCloud.getNumVertices());
    for (int i = 0 ; i < PointCloud.getNumVertices(); i++) {
        // 再定義した特徴量を0.0〜1.0の範囲に正規化
        float tmp = newLinearities[i] / maxNewLinearity;
        
        // 各特徴量（0.0〜1.0）を 0.0〜255.0 に正規化
        float mappedL_new = ofMap(tmp, 0.0, 1.0, 0.0, 255.0);
        
        PointCloud.setColor(i, colorMap.use( (int)mappedL_new ));
        
        mappedNewLinearities.push_back(mappedL_new);
        normalizedNewLinearities.push_back(tmp);
    }
    
    // ファイルに出力
    sort(normalizedLinearities.begin(), normalizedLinearities.end() );
    for (int i = 0; i < normalizedLinearities.size(); i++) {
        oldFeatureValue << normalizedLinearities[i] << endl;
    }
    
    sort(normalizedNewLinearities.begin(), normalizedNewLinearities.end() );
    for (int i = 0; i < normalizedNewLinearities.size(); i++) {
        newFeatureValue << normalizedNewLinearities[i] << endl;
    }
}

// 特徴量分布の変更
void ofApp::changeFeatureValueDistribution(ofVboMesh& PointCloud) {
    for (int i = 0 ; i < PointCloud.getNumVertices(); i++) {
        if (isVisualizeChangeOfCurvature ) PointCloud.setColor(i, colorMap.use( (int)mappedChangeOfCurvatures[i] ));
        
        if (isVisualizePlanarity) PointCloud.setColor(i, colorMap.use( (int)mappedPlanarities[i] ));
        
        if (isVisualizeAplanarity) PointCloud.setColor(i, colorMap.use( (int)mappedAplanarities[i] ));
        
        if (isVisualizeLinearity) PointCloud.setColor(i, colorMap.use( (int)mappedLinearities[i] ));
    }
}
...
void ofApp::displayPCAresult(){
    ...
    // 網掛け
    ofSetColor(255, 150);
    ofDrawRectangle(ofGetWidth()*0.7-10, 100, ofGetWidth()*0.3, 600);
...
//    verdana30.drawString(ofToString(dimentionFeature), ofGetWidth()*0.7, 850);
    ...
    // Change of Curvature
    verdana30.drawString("<Change of Curvature>", ofGetWidth()*0.7, 320);
    verdana30.drawString(ofToString(changeOfCurvature), ofGetWidth()*0.7, 360);
    
    // Planarity
    verdana30.drawString("<Planarity>", ofGetWidth()*0.7, 420);
    verdana30.drawString(ofToString(planarity), ofGetWidth()*0.7, 460);
    
    // Aplanarity
    verdana30.drawString("<Aplanarity>", ofGetWidth()*0.7, 520);
    verdana30.drawString(ofToString(aplanarity), ofGetWidth()*0.7, 560);
    
    // Linearity
    ofSetColor(255, 255, 0);
    verdana30.drawString("<Linearity>", ofGetWidth()*0.7, 610);
    verdana30.drawString(ofToString(linearity), ofGetWidth()*0.7, 650);
    
    // indexes
//    verdana30.drawString("<Indexes>", ofGetWidth()*0.7, 660);
//    verdana30.drawString(ofToString(indexesInSphere), ofGetWidth()*0.1, 700);
#endif
}
...
//            // 特徴量の計算を初期化
//            if (isSecondFeatureValue) {
//                isFirstFeatureValue = false;
//                isSecondFeatureValue = false;
//            }
        }
    }
}
...
void ofApp::calcPlySize(ofVboMesh& PointCloud) {
    ...
    diagonalLineLength = sqrtf(xSize*xSize + ySize*ySize + zSize*zSize);
}
